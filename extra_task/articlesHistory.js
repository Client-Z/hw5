...
;(async () => {
	...
	views.forEach(view => {
		for (let key in logs) {
			console.log(view.articleId, logs[key].meta.articleId)
			// проверить что эта запись лога именно просмотр статьи
			if (view.articleId === +logs[key].meta.articleId && logs[key].timestamp) {
